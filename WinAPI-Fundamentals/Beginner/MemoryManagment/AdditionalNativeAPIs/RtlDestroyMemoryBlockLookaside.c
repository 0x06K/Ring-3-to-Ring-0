...
typedef NTSTATUS (WINAPI *pRtlDestroyMemoryBlockLookaside)(PRTL_MEMORY_BLOCK_LOOKASIDE);

int main() {
    printf("=== RtlDestroyMemoryBlockLookaside ===\n");
    ...
    pRtlDestroyMemoryBlockLookaside RtlDestroyMemoryBlockLookaside = 
        (pRtlDestroyMemoryBlockLookaside)GetProcAddress(ntdll, "RtlDestroyMemoryBlockLookaside");
    
    RTL_MEMORY_BLOCK_LOOKASIDE lookaside = {0};
    RtlCreateMemoryBlockLookaside(&lookaside, 0, 64, 16);
    
    NTSTATUS status = RtlDestroyMemoryBlockLookaside(&lookaside);
