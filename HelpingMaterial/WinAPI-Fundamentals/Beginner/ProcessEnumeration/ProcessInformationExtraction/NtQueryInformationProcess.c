...
typedef NTSTATUS (NTAPI* NtQueryInformationProcess_)(HANDLE, PROCESSINFOCLASS, PVOID, ULONG, PULONG);

int main() {
    HMODULE ntdll = GetModuleHandleW(L"ntdll.dll");
    if (!ntdll) return 1;

    NtQueryInformationProcess_ NtQueryInformationProcess = (NtQueryInformationProcess_)GetProcAddress(ntdll, "NtQueryInformationProcess");
